...
<div class="row">
    ...
    <div class="col-lg-8 mx-auto">
        <div class="card">
            ...
            <div class="card-body">
                <form class="form-horizontal" action="{{ route('category.update', $category_info->id) }}" method="POST" enctype="multipart/form-data">
                	@method('PUT')
                    @csrf
                    <div class="form-group row">
                        <label class="col-md-3 col-form-label">Category Name</label>
                        <div class="col-md-9">
                            <input type="text" placeholder="Category Name" id="name" name="name" class="form-control" required value="{{$category_info->name}}">
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <select class="select2 form-control aiz-selectpicker" name="parent_id" data-toggle="select2" data-placeholder="Choose ..." data-live-search="true">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{$category->id}}" {{$category_info->parent_id==$category->id ? "selected" : ''}}>{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            ...
                            @if ($category_info->banner)
                                <img src="{{asset('images/category')}}/{{$category_info->banner}}" width="50px">
                            @endif
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            ...
                            @if ($category_info->icon)
                                <img src="{{asset('images/category')}}/{{$category_info->icon}}" width="50px">
                            @endif
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <input type="text" value="{{$category_info->meta_title}}" class="form-control" name="meta_title" placeholder="Meta Title">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-md-3 col-form-label">Meta Description</label>
                        <div class="col-md-9">
                            <textarea name="meta_description" rows="5" class="form-control">{{$category_info->meta_description}}</textarea>
                        </div>
                    </div>
