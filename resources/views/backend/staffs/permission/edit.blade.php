...
<div class="card">
    <form action="{{route('permission.update', $permission_name_id->id)}}" method="POST">
        @csrf
        <div class="row card-body">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="groupName">Group Name</label>
                    <input type="text" class="form-control" id="groupName" placeholder="Group Name like Category" name='name' value="{{$permission_name_id->name}}">
                </div>
                @foreach($permissions as $permission)
                <div class="form-group">
                    <label for="permissionCreate">Permission Creaete</label>
                    <input type="text" class="form-control" id="permissionCreate" placeholder="like category.creaete" name='permissionName[]' value='{{$permission->name}}'>
                </div>
                @endforeach
            </div>
            <div class="col-md-6">
